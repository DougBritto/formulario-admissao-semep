...
const OPERATIONS_PATH = path.join(ROOT_DIR, 'config', 'operacoes.json');
...
function loadOperations() {
  ...
  const raw = fs.readFileSync(OPERATIONS_PATH, 'utf8');
  const parsed = JSON.parse(raw);
...
function resolveOperation(config, requestedOperation) {
  const operationKey = String(requestedOperation || '').trim().toLowerCase();
  const operation = operationKey ? config.operations[operationKey] : null;

  if (operationKey && !operation) {
    return {
      requestedKey: operationKey,
      isKnown: false
    };
  }

  const emailTo = operation?.emailTo || config.emailTo;
  const resolved = {
    ...config,
    emailTo,
    emailDestinationLabel: maskEmailForDisplay(emailTo),
    operationKey: operation?.key || '',
    operationLabel: operation?.label || 'Padrão',
    isKnown: true
  };

  resolved.emailConfigured = hasEmailConfiguration(resolved);
  return resolved;
}
...
function validateConfig(config) {
  ...
  const hasOperationEmail = Object.values(config.operations || {}).some((operation) => operation.emailTo);
  if (!config.emailTo && !hasOperationEmail) {
    errors.push('EMAIL_TO deve ser definido no arquivo .env ou em config/operacoes.json.');
  }
...
function createConfig(env = process.env) {
  ...
  const config = {
    ...
    auditRetentionDays: toNumber(env.AUDIT_RETENTION_DAYS, 30),
