...
function normalizeEstadoCivilCode(value) {
  const text = normalizeText(value);
  if (!text) return '';
...
function resolveOptionLabel(options, value) {
  ...
  return match ? normalizeText(match.label, { uppercase: false }) : normalizedValue;
}

function joinPlanoTitulares(payload, dependentes, flag) {
  const nomes = [];

  if (payload?.conjuge_nome && payload[`conjuge_${flag}`]) {
    nomes.push(normalizeText(payload.conjuge_nome));
  }

  dependentes
    .filter((dependente) => dependente && dependente[flag] && dependente.nome)
    .forEach((dependente) => {
      nomes.push(normalizeText(dependente.nome));
    });
...
function transformPayload(payload) {
  ...
  return {
    ...
    sexo_funcionario: normalizeSexoCode(payload.sexo_funcionario),
    estado_civil: normalizeEstadoCivilCode(payload.estado_civil),
    ...
    grau_instrucao: String(payload.grau_instrucao || '').trim(),
    ...
    agencia: normalizeDigits(payload.agencia),
    conta_pagto: normalizePaddedDigits(payload.conta_pagto, 12),
    cpf: normalizeCpf(payload.cpf),
    pis_pasep: normalizeDigits(payload.pis_pasep, 11),
    rg_numero: normalizeText(payload.rg_numero, { uppercase: false }),
    rg_data_emissao: normalizeDate(payload.rg_data_emissao),
    ctps_numero: normalizeDigits(payload.ctps_numero),
    ctps_serie: normalizeDigits(payload.ctps_serie),
    ctps_uf: normalizeText(payload.ctps_uf),
    ctps_data_emissao: normalizeDate(payload.ctps_data_emissao),
    cnh_numero: normalizeDigits(payload.cnh_numero, 11),
    cnh_categoria: normalizeText(cnhCategoria),
    reservista: normalizeDigits(payload.reservista),
    titulo_eleitor: normalizeDigits(payload.titulo_eleitor, 12),
    titulo_zona: normalizeDigits(payload.titulo_zona),
    titulo_secao: normalizeDigits(payload.titulo_secao),
    ...
    cep: normalizeDigits(payload.cep, 8),
    ddd_celular: normalizeDigits(payload.ddd_celular, 2),
    ...
    conjuge_sexo: normalizeSexoCode(payload.conjuge_sexo),
    conjuge_grau_parentesco: String(payload.conjuge_grau_parentesco || '').trim(),
    conjuge_ir: String(payload.conjuge_ir || '').trim(),
...
async function buildWorkbookBuffer(payload, config) {
  ...
  transformed.grau_instrucao = resolveOptionLabel(config.formOptions?.grau_instrucao, transformed.grau_instrucao);
  ...
  config.fieldMapping.dependentes.forEach((mapping, index) => {
    ...
    setCellValue(sheet, mapping.sexo, normalizeSexoCode(dependente.sexo));
    setCellValue(sheet, mapping.grau_parentesco, String(dependente.grau_parentesco || '').trim());
    setCellValue(sheet, mapping.ir, String(dependente.ir || '').trim());
    ...
    setCellValue(sheet, mapping.dnv, normalizeDigits(dependente.dnv, 11));
  ...
  setCellValue(
    ...
    joinPlanoTitulares(payload, dependentes, 'plano_saude')
  );
  setCellValue(
    sheet,
    funcionarioMap.plano_odonto_dependentes,
    joinPlanoTitulares(payload, dependentes, 'plano_odonto')
  );
